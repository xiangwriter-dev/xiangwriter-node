...
`XWComicDirector`；新工作流推荐使用 Source → Planner → Loader，
便于单独排查目录、LLM 和图片加载问题。
...
| 输出场景时长 | agent 识别剧本标题中的“约8秒”“持续10秒”“1.5分钟”等明确标注，一体导演节点从 `duration_seconds` 输出浮点秒数 |
...
4. 把 `duration_seconds`（界面显示“场景时长(秒)”）接到需要 FLOAT 秒数的视频节点；
5. 配置 LLM（见下节）；
6. 点击节点上的按钮：
...
连播会把当前 `prompt_id`、集/段位置和重试次数保存在浏览器本地。页面刷新、标签页休眠、
WebSocket 断线后，插件会查询 `/queue` 与 `/history/{prompt_id}` 恢复监控；只处理本次连播的
执行事件，不会被其它同时运行的工作流误触发。`TD_MiniMax_H3_Prompt` 的连接失败、超时或
空响应会间隔 15 秒自动重试，最多 2 次；显存不足等非临时错误仍会立即停止并显示错误节点。
运行超过 90 分钟时只告警并继续监控，不会擅自中断正在执行的任务。
...
一体导演节点右侧在“段名称”之后新增 `duration_seconds`，界面显示为“场景时长(秒)”，
类型为 `FLOAT`。例如剧本标题写有 `场景 1 日内 立政殿 约 8秒`，节点输出 `8.0`；
写有 `场景时长：1.5分钟`，节点输出 `90.0`，写有 `约8.5秒` 则输出 `8.5`。

时长与选图、提示词在同一次 agent 分析中识别，不会额外调用一次 LLM。
`duration_text` 和 `duration_seconds` 同时保存在 plan 中；插件会校验 `duration_text`
确实存在于剧本原文，并以原文数值为准。没有明确时长标注时输出 `0`，不会主观估算。
LLM 未配置或调用失败时，本地规则仍可识别常见的秒、分钟标注。
...
- **连播中途停止**：点「⏸ 停止连播」。刷新页面或短暂断线后会自动恢复；如果当前工作流
  已报非临时执行错误，修复错误后重新点击连播。
...
仓库不应包含 API Key、`.env`、运行结果、缓存和本地备份；这些内容已加入 `.gitignore`。
推送和 Pull Request 会由 GitHub Actions 自动执行下列检查，本地提交前也建议运行：

~~~text
python tests/test_duration_output.py
node tests/slot_ignore.test.mjs
node tests/auto_advance.test.mjs
